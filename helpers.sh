...
function check_previous_submit() {
    ...
    PREVIOUS=`ssh -o $1 ${RESOURCE} squeue --name=$NAME --user=$FORWARD_USERNAME -o "%R" -h`
...
function get_machine() {
    ...
    while [[ $ALLOCATED == "no" ]]
      do
                                                                  # nodelist
          MACHINE=`ssh -o $1 ${RESOURCE} squeue --name=$NAME --user=$FORWARD_USERNAME -o "%N" -h`
    ...
    echo $MACHINE
    MACHINE="`ssh -o $1 ${RESOURCE} squeue --name=$NAME --user=$FORWARD_USERNAME -o "%R" -h`"
    echo $MACHINE
